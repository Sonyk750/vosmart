...
export default function IncarcareClient({
  ...
  /** Dosarul in care tocmai au intrat documentele, ca sa se poata trece la el. */
  const [dosarulIntrat, setDosarulIntrat] = useState<string | null>(null);
  ...
  async function trimiteTeanc(teanc: FisierAles[]) {
    ...
    if (intrate > 0) {
      ...
      // Dosarul se deschide singur, cu inventarul in el: e singura intrebare pe
      // care o are omul imediat dupa incarcare — „bun, si ce sunt astea?".
      setDosarulIntrat(dosarId);
    ...
    <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      {izbanda && (
        <Card className="rise mt-4 border-ok/30 bg-ok-dim/40 px-4 py-3">
          ...
          {/* Inventarul, verificarea si restul se fac in ecranul de dosare;
              de aici se pleaca spre el, ca omul sa nu-l caute in meniu. */}
          <Link
            href={dosarulIntrat ? `/panou/dosar/${dosarulIntrat}` : "/panou/dosare"}
            className="mt-3 inline-flex items-center gap-1.5 rounded-[var(--radius-field)] border border-line-strong bg-surface-3 px-3.5 py-2 text-[13px] font-medium text-ink transition-colors hover:bg-surface-4"
          >
            Vezi dosarul pe {numeLunaAleasa} {an} <Ic.dreapta className="h-3.5 w-3.5" />
          </Link>
        </Card>
      )}
